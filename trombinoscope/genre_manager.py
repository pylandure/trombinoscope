...
class GenreManager:
  ...
  def openUI(self):
    ...
    current_row = 0
    for genre in genres:
      background_color = odd_background if current_row % 2 == 0 else even_background
      genre_label = tk.Label(self.main_ui, text=genre.genre, width=25, anchor='w',\
                            justify='left', background=background_color)
      genre_label.grid(row=current_row,column=0, sticky='W')

      genre_edit_button = tk.Button(self.main_ui, image=edit_icon, text="Modifier", relief='flat', background=background_color, command=genre.edit)
      genre_edit_button.grid(row=current_row, column=1)
      
      genre_edit_button = tk.Button(self.main_ui, image=delete_icon, text="Supprimer", relief='flat', background=background_color, command=genre.delete)
