...
class StatutManager:
  ...
  def refreshListBox(self):
    ...
    for id_statut in self.statuts:
      statut = self.statuts[id_statut]
  ...
  def refresh(self):
    ...
    Call self.refreshListBox()
    """
    self._statuts = {}
    self._statuts_by_qualification = {}
    return self.refreshListBox()
  ...
  def statuts(self, refresh=False):
    ...
    if len(self._statuts) == 0:      
      sql_queries = ('SELECT id_statut, qualification FROM statut ORDER BY qualification ASC')
      cursor = self.trombinoscope.db.cursor()

      try:
        cursor.execute(sql_queries)
        for (id_statut, qualification) in cursor:
          self._statuts[id_statut] = Statut(self.trombinoscope, id_statut, qualification)
          self._statuts_by_qualification[qualification] = self._statuts[id_statut]
  ...
  def openUI(self):
    ...
    add_icon=tk.PhotoImage(master=self.main_ui, file=self.trombinoscope.config['branding']['image_path'] + '/' + self.trombinoscope.config['branding']['add_icon'])
    edit_icon=tk.PhotoImage(master=self.main_ui, file=self.trombinoscope.config['branding']['image_path'] + '/' + self.trombinoscope.config['branding']['edit_icon'])
    delete_icon=tk.PhotoImage(master=self.main_ui, file=self.trombinoscope.config['branding']['image_path'] + '/' + self.trombinoscope.config['branding']['delete_icon'])

    # Build the bottom buttons frame.
    buttons_frame = tk.Frame(self.main_ui)
    statut_add_button = tk.Button(buttons_frame, image=add_icon, text="Ajouter", command=self.addNewStatut)
    statut_add_button.pack(padx='5px', side=tk.LEFT)
    statut_edit_button = tk.Button(buttons_frame, image=edit_icon, text="Modifier", command=self.editSelectedStatut)
    statut_edit_button.pack(padx='5px', side=tk.LEFT)
    statut_delete_button = tk.Button(buttons_frame, image=delete_icon, text="Supprimer", command=self.deleteSelectedStatut)
    ...
    self.statuts_listbox = tk.Listbox(self.main_ui, yscrollcommand = statuts_listbox_scrollbar.set)
    self.refreshListBox()    
