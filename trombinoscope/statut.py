...
class Statut:
  ...
  def edit(self):
    ...
    save_icon=tk.PhotoImage(master=self.main_ui, file=self.trombinoscope.config['branding']['image_path'] + '/' + self.trombinoscope.config['branding']['save_icon'])
    cancel_icon=tk.PhotoImage(master=self.main_ui, file=self.trombinoscope.config['branding']['image_path'] + '/' + self.trombinoscope.config['branding']['cancel_icon'])

    # Build the bottom buttons frame.
    buttons_frame = tk.Frame(self.main_ui)
    statut_add_button = tk.Button(buttons_frame, image=save_icon, text=save_label, compound=tk.LEFT, command=self.update)
    statut_add_button.pack(padx='5px', side=tk.LEFT)
    statut_edit_button = tk.Button(buttons_frame, image=cancel_icon, text="Annuler", compound=tk.LEFT, command=self.main_ui.destroy)
  ...
  def delete(self):
    ...
    """
    answer=messagebox.askquestion('Supprimer le statut n°{id_statut} ?'.format(id_statut=self.id_statut),
                      'Voulez-vous vraiment supprimer le statut \'{qualification}\' ?'.format(qualification=self.qualification),
                      icon = 'warning', parent=self.trombinoscope.statut_manager.main_ui)
    if answer == 'no':
      return False
    ...
    try:
      ...
      # Commit the change.
      self.trombinoscope.db.commit()
    except MysqlError as mysql_error:
      # Affiche une alerte pour l'erreur.
      error_message = 'Impossible de supprimer le statut :\n' + str(mysql_error)
      messagebox.showerror('Erreur', error_message, parent=self.trombinoscope.statut_manager.main_ui)
    finally:
      cursor.close()
  ...
  def update(self):
    ...
    if len(updated_qualification) == 0:
      messagebox.showerror('Erreur', 'La qualification est obligatoire !', parent=self.trombinoscope.statut_manager.main_ui)
    ...
    if self.id_statut is not None:
      sql_queries = ('UPDATE statut SET qualification = %(qualification)s WHERE id_statut=%(id_statut)s')
      sql_queries_params['id_statut'] = self.id_statut
